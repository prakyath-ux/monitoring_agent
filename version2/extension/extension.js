...
// ---- Thin Loader: single setup function ----
// Replaces ensureAgentInstalled() + ensureProjectInit()
// All heavy lifting (venv, deps, .agent init, service) delegated to setup.py
...
function ensureSetup(cwd) {
    ...
    // Step 1: Clone repo if needed (only thing extension does directly)
    if (!fs.existsSync(path.join(AGENT_HOME, '.git'))) {
        updateStatusBar('installing');
        vscode.window.showInformationMessage('Agent Monitor: Cloning repository...');

        try {
            // Remove broken directory if it exists but isn't a git repo
            ...
            vscode.window.showErrorMessage(`Agent Monitor: Clone failed - ${e.message}`);
            updateStatusBar('stopped');
            return false;
        }
    }

    // Step 2: Run setup.py for everything else (venv, deps, .agent init, service)
    const setupPy = path.join(AGENT_HOME, 'version2', 'setup.py');
    if (!fs.existsSync(setupPy)) {
        vscode.window.showErrorMessage('Agent Monitor: setup.py not found in repository.');
        updateStatusBar('stopped');
        return false;
    }
...
// ---- Streamlit launch (unchanged from POC) ----
...
async function startAgent() {
    ...
    // Skip extension repo itself
    if (fs.existsSync(path.join(cwd, 'extension.js'))) {
        updateStatusBar('stopped');
        return;
    }

    // If .agent/ exists, ensure install is good and launch
